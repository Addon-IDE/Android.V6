类 其他_附件_行为包管理器 : 窗口
	...
	变量 线性布局3 : 线性布局
	@布局配置([[父布局=线性布局2,@左外边距DP=10,粗体=真,字体大小=16,内容="行为包管理器",字体颜色=-16777216]])
	变量 文本框3 : 文本框
	...
	方法 加载列表(路径 : 文本,刷新 : 逻辑型 = 假)
		...
		否则
			...
			事件 自定义列表框1:加载布局(索引 : 整数,项目布局 : 组件容器) : 组件容器
				变量 布局 : 其他_附件_行为包管理器_列表?
				如果 项目布局 == 空 则
					布局 = 创建 其他_附件_行为包管理器_列表
				否则
					布局 = 项目布局
				结束 如果
				//显示到列表
				变量 文件路径 : 文本 = 列表集合[索引]
				//加载图标
				如果 文件是否存在(文件路径 + "/pack_icon.png") 则
					布局.图片框1.图片路径 = 文件路径 + "/pack_icon.png"
				结束 如果
				//加载清单
				如果 文件是否存在(文件路径 + "/manifest.json") 则
					变量 根对象 : JSON对象 = 读入文本文件(文件路径 + "/manifest.json")
					变量 header : JSON对象 = 根对象["header"]
					变量 name : 文本 = header["name"]
					变量 description : 文本 = header["description"]
					布局.文本框1.内容 = name
					布局.文本框2.内容 = description
				否则
					布局.文本框1.内容 = "清单文件丢失!"
					布局.文本框2.内容 = "清单文件丢失，无法获取"
				结束 如果
				//列表被点击
				事件 布局.线性布局2:被单击()
					列表单击(文件路径,布局)
				...
				事件 布局.线性布局2:被长按()
					...
					变量 header : JSON对象 = 根对象["header"]
					变量 name : 文本 = header["name"]
					删除提示.内容("是否删除" + name + ",这个将会消除很久,不可恢复,是否要继续")
					删除提示.按钮1("删除")
					删除提示.按钮2("取消")
					删除提示.显示()
					事件 删除提示:按钮1被单击() : 逻辑型
	...
	方法 列表单击(路径 : 文本,布局 : 其他_附件_行为包管理器_列表)
		变量 详细布局 : 其他_附件_行为包管理器_详细
		变量 详细菜单 : DX菜单
		详细菜单.自定义布局(详细布局)
		详细菜单.依附组件(布局.线性布局1)
		详细菜单.依附位置(DX对齐方式.居中)
		...
		如果 设置_设定.获取开关("NightMode") 则
			详细菜单.主题(DX主题.暗色)
			详细布局.线性布局1.背景颜色 = 0XFF333333
			详细布局.文本框1.字体颜色 = 0XFFFFFFFF
			详细布局.文本框2.字体颜色 = 0XFFFFFFFF
		结束 如果
		详细菜单.宽度(1000)
		详细菜单.显示()
		//加载图标
		如果 文件是否存在(路径 + "/pack_icon.png") 则
			详细布局.图片框1.图片路径 = 路径 + "/pack_icon.png"
		结束 如果
		//加载清单数据
		如果 文件是否存在(路径 + "/manifest.json") 则
			变量 根对象 : JSON对象 = 读入文本文件(路径 + "/manifest.json")
			变量 header : JSON对象 = 根对象["header"]
			变量 name : 文本 = header["name"]
			变量 version : 文本 = header["version"].到文本()
			变量 description : 文本 = header["description"]
			详细布局.文本框1.内容 = name
			详细布局.文本框2.内容 = version.替换("[","").替换("]","")
			详细布局.文本框3.内容 = description
		否则
			详细布局.文本框1.内容 = "获取异常,清单文件丢失"
			详细布局.文本框2.内容 = "获取异常,清单文件丢失"
			详细布局.文本框3.内容 = "获取异常,清单文件丢失"
		结束 如果
	结束 方法
...
类 其他_附件_行为包管理器_详细 : 组件容器
	...
	@布局配置([[父布局=线性布局4,@左外边距DP=5,字体大小=13,跑马灯效果=真]])
	变量 文本框2 : 文本框
	@布局配置([[父布局=线性布局1,宽度=-1,高度DP=80]])
	变量 滚动布局1 : 滚动布局
	@布局配置([[父布局=滚动布局1,宽度=-1,高度DP=80,对齐方式=8388659]])
	变量 线性布局6 : 线性布局
	@布局配置([[父布局=线性布局6,@左外边距DP=5,@上外边距DP=5,字体颜色=-14435585]])
	变量 文本框3 : 文本框
