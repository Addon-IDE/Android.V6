...
类 ProgressDialog
	...
	变量 dialog : 对话框?
	@隐藏
	变量 Layout : AlertProgressDialogLayout?
	@隐藏
	变量 NumberButton : 整数
	@隐藏
	...
	属性写 初始化(环境名 : 安卓环境)
		...
		Layout = 新建窗口组件(AlertProgressDialogLayout,安卓环境名)
		//设置组件
		dialog.自定义布局 = Layout
	...
	属性写 进度(进度值 : 整数)
		Progress = 进度值
		Layout.进度条1.进度 = 进度值
		Layout.文本框6.内容 = 获取百分号(Progress,Max) + "%"
		Layout.文本框7.内容 = Progress + "/" + Max
	结束 属性
...
类 AlertProgressDialogLayout : 组件容器
	...
	变量 文本框2 : 文本框
	@布局配置([[父布局=线性布局2,宽度=-1,高度=-2,右内边距DP=20,左内边距DP=20,纵向布局=真]])
	变量 线性布局10 : 线性布局
	@布局配置([[父布局=线性布局10,宽度=-1,高度=-2,@上外边距DP=10]])
	变量 进度条1 : 进度条
	@布局配置([[父布局=线性布局10,宽度=-1,高度DP=25]])
	变量 相对布局2 : 相对布局
	@布局配置([[父布局=相对布局2,宽度=-1,高度=-1]])
	变量 线性布局9 : 线性布局
	@布局配置([[父布局=线性布局9,@上外边距DP=5,内容="0%",单行显示=真,粗体=真]])
	变量 文本框6 : 文本框
	@布局配置([[父布局=相对布局2,宽度=-1,高度=-1,对齐方式=8388629]])
	变量 线性布局11 : 线性布局
	@布局配置([[父布局=线性布局11,内容="0/100",单行显示=真,粗体=真]])
	变量 文本框7 : 文本框
